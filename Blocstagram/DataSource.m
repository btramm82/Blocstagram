...
-(void) requestNewItemsWithCompletionHandler:(NewItemCompletionBlock)completionHandler {
    ...
    if (self.isRefreshing == NO) {
        ...
        NSString *minID = @"notused";
        if([[self.mediaItems firstObject] idNumber]) {
...
NSMutableArray *mutableArrayWithKVO = [self mutableArrayValueForKey:@"mediaItems"];
    ...
    if (tmpMediaItems.count > 0) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSUInteger numberOfItemsSave = MIN(self.mediaItems.count, 50);
            NSArray *mediaItemsToSave = [self.mediaItems subarrayWithRange:NSMakeRange(0, numberOfItemsSave)];
